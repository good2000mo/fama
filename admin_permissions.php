...
if (isset($_POST['form_sent']))
{
	...
	foreach ($form as $key => $input)
	{
		...
		if (array_key_exists('p_'.$key, $pun_config) && $pun_config['p_'.$key] != $input)
			$db->query('UPDATE '.$db->prefix.'config SET conf_value='.$input.' WHERE conf_name=\'p_'.$db->escape($key).'\''.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to update board config', __FILE__, __LINE__, $db->error());
	}
