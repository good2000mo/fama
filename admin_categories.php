...
if (isset($_POST['add_cat']))
{
	...
	$db->query('INSERT INTO '.$db->prefix.'categories (cat_name) VALUES(\''.$db->escape($new_cat_name).'\')'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to create category', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['del_cat']) || isset($_POST['del_cat_comply']))
{
	...
	if (isset($_POST['del_cat_comply'])) // Delete a category with all forums and posts
	{
		...
		$result = $db->query('SELECT id FROM '.$db->prefix.'forums WHERE cat_id='.$cat_to_delete.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to fetch forum list', __FILE__, __LINE__, $db->error());
		...
		for ($i = 0; $i < $num_forums; ++$i)
		{
			...
			// Delete the forum
			$db->query('DELETE FROM '.$db->prefix.'forums WHERE id='.$cur_forum.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to delete forum', __FILE__, __LINE__, $db->error());
		}
		...
		// Delete the category
		$db->query('DELETE FROM '.$db->prefix.'categories WHERE id='.$cat_to_delete.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to delete category', __FILE__, __LINE__, $db->error());
	...
	{
		$result = $db->query('SELECT cat_name FROM '.$db->prefix.'categories WHERE id='.$cat_to_delete.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to fetch category info', __FILE__, __LINE__, $db->error());
...
else if (isset($_POST['update'])) // Change position and name of the categories
{
	...
	foreach ($categories as $cat_id => $cur_cat)
	{
		...
		$db->query('UPDATE '.$db->prefix.'categories SET cat_name=\''.$db->escape($cur_cat['name']).'\', disp_position='.$cur_cat['order'].' WHERE id='.intval($cat_id).' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to update category', __FILE__, __LINE__, $db->error());
	}
...
$result = $db->query('SELECT id, cat_name, disp_position FROM '.$db->prefix.'categories ORDER BY disp_position'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
