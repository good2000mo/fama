...
function update_search_index($mode, $post_id, $message, $subject = null)
{
	...
	if ($mode == 'edit')
	{
		$result = $db->query('SELECT w.id, w.word, m.subject_match FROM '.$db->prefix.'search_words AS w INNER JOIN '.$db->prefix.'search_matches AS m ON w.id=m.word_id WHERE m.post_id='.$post_id.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --', true) or fama_error('Unable to fetch search index words', __FILE__, __LINE__, $db->error());
	...
	{
		$result = $db->query('SELECT id, word FROM '.$db->prefix.'search_words WHERE word IN(\''.implode('\',\'', array_map(array($db, 'escape'), $unique_words)).'\')'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --', true) or fama_error('Unable to fetch search index words', __FILE__, __LINE__, $db->error());
		...
		if (!empty($new_words))
		{
			switch ($db_type)
			{
				...
				case 'mysql_innodb':
				case 'mysqli_innodb':
					$db->query('INSERT INTO '.$db->prefix.'search_words (word) VALUES(\''.implode('\'),(\'', array_map(array($db, 'escape'), $new_words)).'\')'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --');
					break;

				default:
					foreach ($new_words as $word)
						$db->query('INSERT INTO '.$db->prefix.'search_words (word) VALUES(\''.$db->escape($word).'\')'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --');
					break;
			}
		}
	...
	foreach ($words['del'] as $match_in => $wordlist)
	{
		...
		if (!empty($wordlist))
		{
			...
			$db->query('DELETE FROM '.$db->prefix.'search_matches WHERE word_id IN('.$sql.') AND post_id='.$post_id.' AND subject_match='.$subject_match.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or fama_error('Unable to delete search index word matches', __FILE__, __LINE__, $db->error());
		}
	}
...
function strip_search_index($post_ids)
{
	...
	switch ($db_type)
	{
		...
		case 'mysqli_innodb':
		{
			$result = $db->query('SELECT word_id FROM '.$db->prefix.'search_matches WHERE post_id IN('.$post_ids.') GROUP BY word_id'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or fama_error('Unable to fetch search index word match', __FILE__, __LINE__, $db->error());
			...
			if ($db->num_rows($result))
			{
				...
				$result = $db->query('SELECT word_id FROM '.$db->prefix.'search_matches WHERE word_id IN('.$word_ids.') GROUP BY word_id HAVING COUNT(word_id)=1'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or fama_error('Unable to fetch search index word match', __FILE__, __LINE__, $db->error());
				...
				if ($db->num_rows($result))
				{
					...
					$db->query('DELETE FROM '.$db->prefix.'search_words WHERE id IN('.$word_ids.')'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or fama_error('Unable to delete search index word', __FILE__, __LINE__, $db->error());
				}
			}
		...
		default:
			$db->query('DELETE FROM '.$db->prefix.'search_words WHERE id IN(SELECT word_id FROM '.$db->prefix.'search_matches WHERE word_id IN(SELECT word_id FROM '.$db->prefix.'search_matches WHERE post_id IN('.$post_ids.') GROUP BY word_id) GROUP BY word_id HAVING COUNT(word_id)=1)'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or fama_error('Unable to delete from search index', __FILE__, __LINE__, $db->error());
			break;
	}

	$db->query('DELETE FROM '.$db->prefix.'search_matches WHERE post_id IN('.$post_ids.')'.' -- sqlcomment: '.__FILE__.' line:'.__LINE__.' --') or fama_error('Unable to delete search index word match', __FILE__, __LINE__, $db->error());
}
